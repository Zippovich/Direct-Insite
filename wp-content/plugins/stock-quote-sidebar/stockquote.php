...
/*  Copyright 2011  Andrew Hill  (email : andy@hillhome.org)
...
function sqsb_plugin_action_links( $links, $file ) {
	if ( $file == plugin_basename( dirname(__FILE__).'/stockquote.php' ) ) {
		$settings_link = '<a href="' . get_bloginfo('wpurl') . '/wp-admin/options-general.php?page=stock-quote-sidebar/stockquote.php">'.__('Settings').'</a>';
...
add_filter( 'plugin_action_links', 'sqsb_plugin_action_links', 10, 2 );
...
<form name="stockquotesidebarform" action="$action_url" method="post">
        ...
        <li>Specify a list of symbols in the php function within your WordPress template<br/>
            ...
            <code>&lt;?php get_stock_quote("^IXIC,^GSPC,NOVL,ko,pfe,intc"); ?&gt;</li></code></ol>
  <br/>
        ...
        <p>A usage example would be to put indexes such as ^GSPC or ^FTSE, as well as stocks that you hold or want to track closely in the first list, and other less interesting stocks in the second list.</p>
		<p><b>Note:</b> Yahoo no longer provides the Dow Jones Industrial Average via CSV download, due to a restriction imposed by Dow Jones.  Therefore, you can not use ^DJI or INDU with this plugin.</p>
...
if($passedsymbols == "getfromdb") {
	...
	// Use the symbols passed with PHP, and sub the INDU symbol
    $stocksymbols = str_replace("^DJI", "INDU", $passedsymbols);
    }
...
//            }
            ...
            if ($stocksymbol == "SQSBSEPARATOR") {
                echo '<td colspan="8"><hr id="sqsbhr" noshade="noshade" size="1"/></td>';
            }
            else {
...
//	       			echo "<td width='60'><a title='$company' href='$sqclickurl' target='_blank'>$displaystocksymbol</a></td>";
	       			echo "<td width='50' align='left'>$last</td>";
//	       			echo "<td>&nbsp;&nbsp;</td>";
    					// Begin Points or percent block
							if ($pointsorpercent == "percent") {
    						if($change < 0) {
	       					echo "<td style='white-space: nowrap;' width='45' align='left'><a href='$chartclickurl' class='sqsbchart'><img alt='chart' src='$charturl' border='0' width='192' height='96'/><font color='red'>";
	       					printf("%+8.2f%%", $percentchange);
	       					echo "</font></a></td>";
	       				}
	       				else if($change > 0) {
	       					echo "<td style='white-space: nowrap;' width='45' align='left'><a class='sqsbchart' href='$chartclickurl'><img alt='chart' src='$charturl' border='0' width='192' height='96'/><font color='green'>";
	       					printf("%+8.2f%%", $percentchange);
	       					echo "</font></a></td>";
	       				}
	       				else {
	       					echo "<td style='white-space: nowrap;' width='45' align='left'><a class='sqsbchart' href='$chartclickurl'><img alt='chart' src='$charturl' border='0' width='192' height='96'/>";
