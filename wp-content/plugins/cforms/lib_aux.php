...
function check_cust_vars($m,$t,$no) {
	...
	if ( count($findall[1]) > 0 ) {
		...
		foreach ( $findall[1] as $fvar ) {
			...
			if (preg_match('/(cf_form\d?_\$\$\$)/',$key, $match)) {
				$key = $match[1];
			}
			if (preg_match('/(cf_form\d?_)/',$key, $match2)) {
				$key2 = $match2[1];
			}
			...
			if ( strpos($fvar,'_field')!==false ){
				...
				if ( $allvars[$fNo]<>'' ) {
					$fTrackedVar = $t[$key.$fNo];  ### reset $fvar to actual label name and continue
				}
			}

			### convert if alt [id:] used
			if ( in_array( '$$$'.$fTrackedVar, $allvars ) ){
				if ( $t[$key.$fTrackedVar]<>'' )
					$fTrackedVar = $t[$key.$fTrackedVar];  ### reset $fvar to actual label name and continue
			}

			### check if label name is tracked...
			if( in_array( $key2.$fTrackedVar,$allvars ) )
				$m = str_replace('{'.$fvar.'}', stripslashes($t[$key2.$fTrackedVar]), $m);
