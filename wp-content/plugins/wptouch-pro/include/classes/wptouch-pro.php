		...
		foreach( $include_params as $include_param ) {
			...
			if ( preg_match_all( $reg_ex, $file_contents, $match ) ) {
				...
				//		$file_contents = str_replace( $statement_in_code_that_loads_file, $new_statement, $file_contents );					
					...
					//echo $statement_in_code_that_loads_file;
				}
			}
		}			
...
//			echo "<![endif]-->\n";
	...
	function get_admob_ad() {
		...
		if ( $this->active_device == 'iphone' ) {
			...
			include( get_wptouch_directory() . '/include/advertising/admob-mobile-web.php' );
		}
	...
	function check_plugins_for_warnings() {
		...
		if ( WPTOUCH_SIMULATE_ALL || function_exists( 'wp_super_cache_init' ) ) {
			$this->add_warning( 'WP Super Cache', __('Configuration is required to work with WPtouch Pro. It must configured to exclude the user agents that WPtouch Pro is enabled for (iphone, ipod, aspen, incognito, webmate, android, dream, cupcake, froyo, blackberry9500, blackberry9520, blackberry9530, blackberry9550, blackberry 9800, blackberry 9850, blackberry 9860, webos, s8000, bada)', 'wptouch-pro' ),  'http://www.bravenewcode.com/support/topic/using-wptouch-pro-with-wp-super-cache' );	
		}
		
		if ( WPTOUCH_SIMULATE_ALL || class_exists( 'W3_Plugin_TotalCache' ) ) {
			$this->add_warning( 'W3 Total Cache', __('Extra configuration is required. It must be configured to exclude the user agents that WPtouch Pro is enabled for (iphone, ipod, aspen, incognito, webmate, android, dream, cupcake, froyo, blackberry9500, blackberry9520, blackberry9530, blackberry9550, blackberry 9800, blackberry 9850, blackberry 9860, webos, s8000, bada)', 'wptouch-pro' ), 'http://www.bravenewcode.com/support/topic/using-wptouch-pro-with-w3-cache' );	
		}
...
//		}
	...
	function admin_ajax_handler() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			switch( $wptouch_ajax_action ) {
				...
				case 'set-menu-icon':
					$settings = $this->get_settings();
					$settings->temp_menu_icons[ $this->post['title'] ] = bnc_wptouch_sslize( str_replace( WP_CONTENT_URL, '', $this->post['icon'] ) );
					$this->save_settings( $settings );
					break;
	...
	function add_mobile_header_info() {
		...
		$minfile = WP_CONTENT_DIR . $settings->current_theme_location . '/' . $settings->current_theme_name . '/' . $this->get_active_device_class() . '/style.min.css';
		if ( file_exists( $minfile ) ) {		
