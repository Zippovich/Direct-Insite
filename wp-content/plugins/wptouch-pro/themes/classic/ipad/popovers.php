				...
				<form name="loginform" id="loginform" action="<?php wptouch_bloginfo('wpurl'); ?>/wp-login.php?redirect_to=<?php echo urlencode( wptouch_get_current_page_url() ); ?>" method="post">
					...
					<?php if ( classic_ipad_accounts_enabled() ) { ?>
						<div id="account-link-area">
							<p><?php echo sprintf( __( "Not registered yet?<br />You can %ssign-up here%s.", "wptouch-pro" ), '<a class="no-ajax" href="' . wptouch_bloginfo( 'wpurl' ) . '/wp-register.php">','</a>' ); ?></p>
							<p><?php echo sprintf(__( "Lost your password?<br />You can %sreset it here%s.", "wptouch-pro" ), '<a class="no-ajax" href="' . wptouch_bloginfo( 'wpurl' ) . '/wp-login.php?action=lostpassword">','</a>' ); ?></p>				</div>
					<?php } ?>
			</form>
			<?php } ?>		
		</div>
